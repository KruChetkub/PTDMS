-- Add application-specific satisfaction surveys while preserving the original SmartDSP survey.

begin;

drop index if exists public.idx_smartdsp_surveys_one_enabled_active;

create unique index if not exists idx_smartdsp_surveys_one_enabled_active_per_code
on public.smartdsp_surveys (code)
...
select
  target.id,
  source_option.rating_value,
  source_option.label,
  source_option.description
from public.smartdsp_surveys target
join public.smartdsp_surveys source
  on source.code = 'smartdsp-satisfaction'
 and source.version = 1
join public.smartdsp_survey_rating_options source_option
  on source_option.survey_id = source.id
