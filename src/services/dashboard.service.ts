...
export async function getDashboardSummary(): Promise<DashboardSummary> {
  ...
  const totalCategorized = records.filter((record) => trainingTypeOptions.includes(record.category as (typeof trainingTypeOptions)[number])).length;
  const categoryBreakdown = trainingTypeOptions.map((label) => {
    const count = records.filter((record) => record.category === label).length;
    return {
      label,
      count,
      percentage: totalCategorized > 0 ? Math.round((count / totalCategorized) * 100) : 0,
      topCourse: topCourseByCategory(records, label),
    };
  });
