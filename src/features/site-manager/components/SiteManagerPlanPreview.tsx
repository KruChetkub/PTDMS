    ...
    <section className="rounded-md border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
      ...
      <div className="mt-5 overflow-x-auto pb-2">
        {visibleCards.length > 0 ? (
            ...
            {visibleCards.map(({ card, cardIndex }) => {
              ...
              return (
                <button
                  ...
                  className="flex min-h-64 w-[180px] shrink-0 flex-col rounded-md border border-emerald-200 bg-white p-4 text-center text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:border-emerald-400 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
                >
                  {card.coverImageUrl ? (
          ...
          <div className="rounded-md border border-dashed border-emerald-300 bg-white px-4 py-6 text-sm text-emerald-700">
            ยังไม่มีรายการที่เผยแพร่ใน{title}
          </div>
        )}
      </div>
    </section>
