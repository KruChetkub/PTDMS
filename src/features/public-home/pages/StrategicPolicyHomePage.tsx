...
const policyToneMap: Record<PublicHomeColorKey, string> = {
  ...
  rose: 'border-pink-200 bg-pink-50/60 text-pink-700',
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      <header className="border-b border-emerald-200 bg-white">
        <div className="mx-auto grid max-w-[1600px] gap-5 px-4 py-5 sm:px-6 lg:grid-cols-[11rem_minmax(0,1fr)_auto] lg:items-center lg:px-8">
          ...
          <Link to="/strategic-repository" className="inline-flex h-11 items-center justify-center gap-2 rounded-md bg-[#0b7750] px-4 text-sm font-semibold text-white transition hover:bg-[#086340]">
            <BookOpenText className="h-5 w-5" aria-hidden="true" />
            คลังเอกสารยุทธศาสตร์
          </Link>
        </div>
      </header>
      ...
      <main className="relative isolate overflow-hidden bg-[linear-gradient(180deg,#F4FBFF_0%,#E9F8FA_46%,#F7FBFF_100%)]">
          ...
          <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
              <div className="shrink-0 lg:w-48">
                <p className="text-3xl font-bold uppercase text-blue-700">Sustainable</p>
                <p className="text-l font-bold uppercase text-blue-700">Development Goals</p>
                <p className="mt-1 text-m font-medium text-slate-1000">เป้าหมายการพัฒนาที่ยั่งยืน</p>
              </div>
        ...
        {visibleSections.map((section, sectionIndex) => {
          ...
          return (
            <section key={section.id} className={`border-b border-slate-200/70 py-7 ${isPrimaryPlanSection ? 'bg-white/80' : 'bg-transparent'}`}>
              <div className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
                ...
                {isPrimaryPlanSection ? (
                  ...
                  >
                    {items.map((item) => {
                      ...
                      return <Link key={item.id} to={item.to} className="group flex min-h-52 w-[78%] shrink-0 snap-start flex-col items-center rounded-md border border-slate-200 bg-white p-4 text-center transition hover:border-emerald-200 hover:bg-slate-50 sm:w-[42%] md:w-[30%] lg:w-[calc((100%_-_5.25rem)/8)]">
                        {item.logoUrl ? <img src={item.logoUrl} alt={`โลโก้ ${item.title}`} className="h-12 w-12 rounded-full border-2 border-white object-cover shadow-md ring-1 ring-slate-200" /> : <span className={`flex h-11 w-11 items-center justify-center rounded-full text-white ${item.color}`}><Icon className="h-5 w-5" aria-hidden="true" /></span>}
                    ...
                    {items.map((item, itemIndex) => {
                      ...
                      return <div key={item.id} className="flex min-w-0 flex-col">
                        <div className="mb-3 flex items-center gap-3 px-1">
                          <span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-base font-bold text-white ${item.color}`}>{itemIndex + 2}</span>
                        ...
                        <article className={`flex-1 rounded-md border p-5 ${item.tone}`}>
                          <div className="flex items-start gap-4">
                            ...
                            <div className="min-w-0 flex-1"><p className="text-sm leading-6 text-slate-700">{item.description}</p><Link to={item.to} className="mt-4 inline-flex h-9 items-center gap-2 rounded-md border border-current bg-white px-3 text-sm font-semibold hover:bg-white/60">{item.actionLabel}<ArrowRight className="h-4 w-4" /></Link></div>
                          </div>
                        </article>
                      </div>;
                    })}
